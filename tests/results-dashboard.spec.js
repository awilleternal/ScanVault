...
test.describe('Results Dashboard Functionality', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    if (testInfo.status !== testInfo.expectedStatus) {
      await takeTimestampedScreenshot(
        page,
        `results-dashboard-failure-${testInfo.title}`
      );
    }
  ...
  test('should display results dashboard with summary statistics', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('h2', { hasText: 'Security Scan Results Summary' })
    ).toBeVisible();
  ...
  test('should display tab navigation with correct counts', async ({
    page,
  }) => {
    // Verify overview tab
    ...
    await expect(overviewTab).toHaveClass(
  ...
  test('should switch between result tabs correctly', async ({ page }) => {
    ...
    await expect(page.locator('button', { hasText: /Semgrep/ })).toHaveClass(
    ...
    await expect(
      page.locator(
        'text=finds security vulnerabilities, bugs, and anti-patterns'
      )
    ).toBeVisible();
    ...
    await expect(page.locator('button', { hasText: /Trivy/ })).toHaveClass(
      /border-green-500|text-green-600/
    );

    // Should show Trivy description
    await expect(
      page.locator('text=scans your dependencies and container images')
    ).toBeVisible();

    // Switch back to overview
    await securityScannerPage.switchToTab('overview');
    await expect(page.locator('button', { hasText: /Overview/ })).toHaveClass(
      /border-primary-500|text-primary-600/
    );
  });
  ...
  test('should display vulnerability cards with correct information', async ({
    page,
  }) => {
    ...
    await expect(
      firstCard.locator('text=/CRITICAL|HIGH|MEDIUM|LOW|INFO/')
    ).toBeVisible();
  ...
  test('should show detailed information in expanded cards', async ({
    page,
  }) => {
    const resultCount = await securityScannerPage.getResultsCount();
  ...
  test('should handle empty results gracefully', async ({ page }) => {
    ...
    await expect(page.locator('text=No security issues found')).toBeVisible({
      timeout: 10000,
    });
    ...
    await page.route('**/api/scan/*/report/json', (route) => {
      route.fulfill({
        ...
        headers: {
          ...
          'content-disposition':
            'attachment; filename="security-scan-test-scan-id.json"',
        },
        body: JSON.stringify({
          scanId: 'test-scan-id',
    ...
    await page.route('**/api/scan/*/report/pdf', (route) => {
      route.fulfill({
        ...
        headers: {
          ...
          'content-disposition':
            'attachment; filename="security-scan-test-scan-id.pdf"',
        },
  ...
  test('should display tool-specific styling and icons', async ({ page }) => {
    ...
    await expect(
      page.locator('text=🔍').or(page.locator('text=Semgrep'))
    ).toBeVisible();
    await expect(
      page.locator('text=📦').or(page.locator('text=Trivy'))
    ).toBeVisible();

    // Should have tool-specific color coding
    const semgrepElements = page.locator(
      '.border-blue-500, .bg-blue-50, .text-blue-600'
    );
    const trivyElements = page.locator(
      '.border-green-500, .bg-green-50, .text-green-600'
    );

    // At least one should be visible if we have results from both tools
    const hasSemgrepStyling = await semgrepElements
      .first()
      .isVisible()
      .catch(() => false);
    const hasTrivyStyling = await trivyElements
      .first()
      .isVisible()
      .catch(() => false);
  ...
  test('should show appropriate loading states during data fetching', async ({
    page,
  }) => {
    ...
    const loadingElements = page.locator(
      '.animate-spin, .animate-pulse, text=Loading'
    );

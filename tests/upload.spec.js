...
import { createTestZipFile, cleanupTestFiles, takeTimestampedScreenshot } from './utils/testHelpers.js';
...
test.describe('File Upload Functionality', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    if (testInfo.status !== testInfo.expectedStatus) {
      await takeTimestampedScreenshot(page, `upload-test-failure-${testInfo.title}`);
    }
  ...
  test('should display file upload interface by default', async ({ page }) => {
    ...
    await expect(securityScannerPage.uploadTypeToggle.file).toHaveClass(/bg-primary-600|text-white/);
    await expect(securityScannerPage.dropzone).toBeVisible();
    
    // Verify dropzone instructions
    await expect(page.locator('text=Drag and drop a ZIP file here')).toBeVisible();
  ...
  test('should switch between upload types', async ({ page }) => {
    ...
    await expect(securityScannerPage.uploadTypeToggle.file).toHaveClass(/bg-primary-600/);
    
    // Switch to URL upload
    await securityScannerPage.uploadTypeToggle.url.click();
    await expect(securityScannerPage.uploadTypeToggle.url).toHaveClass(/bg-primary-600/);
  ...
  test('should successfully upload a valid ZIP file', async ({ page }) => {
    ...
    await securityScannerPage.waitForToast('File uploaded successfully', 'success');
  ...
  test('should show upload progress during file upload', async ({ page }) => {
    ...
    const testZipPath = path.join(__dirname, 'fixtures', 'large-test-upload.zip');
    ...
    await expect(securityScannerPage.uploadProgress.or(page.locator('text=Uploading'))).toBeVisible({ timeout: 5000 });
  ...
  test('should reject files larger than 100MB', async ({ page }) => {
    ...
    // Should show error toast
    await securityScannerPage.waitForToast('File size must be less than 100MB', 'error');
  ...
  test('should disable upload during processing', async ({ page }) => {
    ...
    await expect(securityScannerPage.dropzone).toHaveClass(/opacity-50|cursor-not-allowed/);

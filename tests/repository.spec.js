...
import { takeTimestampedScreenshot, mockApiResponse, simulateSlowNetwork, resetNetworkConditions } from './utils/testHelpers.js';
...
test.describe('Repository URL Functionality', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    if (testInfo.status !== testInfo.expectedStatus) {
      await takeTimestampedScreenshot(page, `repository-test-failure-${testInfo.title}`);
    }
  });

  test('should display repository URL interface', async ({ page }) => {
    // Verify URL mode is selected
    await expect(securityScannerPage.uploadTypeToggle.url).toHaveClass(/bg-primary-600|text-white/);
    ...
    await expect(page.locator('label[for="repositoryUrl"]')).toHaveText('Repository URL');
    await expect(page.locator('text=Supports GitHub, GitLab, Bitbucket')).toBeVisible();
    
    // Verify placeholder text
    await expect(securityScannerPage.repositoryUrlInput).toHaveAttribute('placeholder', /github\.com|repository/);
  });
  ...
  test('should successfully clone a valid repository', async ({ page }) => {
    ...
    await securityScannerPage.waitForToast('Repository cloned successfully', 'success');
  ...
  test('should handle repository clone with loading state', async ({ page }) => {
    ...
    // Fill repository URL
    await securityScannerPage.repositoryUrlInput.fill(testRepositories.smallRepository);
    ...
    await expect(securityScannerPage.cloneRepositoryButton).toHaveText(/Processing/);
  ...
  test('should handle repository not found error', async ({ page }) => {
    ...
    await securityScannerPage.repositoryUrlInput.fill(testRepositories.nonExistentRepository);
    await securityScannerPage.cloneRepositoryButton.click();
  ...
  test('should handle authentication required error', async ({ page }) => {
    ...
    await securityScannerPage.repositoryUrlInput.fill('https://github.com/private/repository.git');
    await securityScannerPage.cloneRepositoryButton.click();
  ...
  test('should handle network timeout gracefully', async ({ page }) => {
    ...
    await securityScannerPage.repositoryUrlInput.fill(testRepositories.validRepository);
    await securityScannerPage.cloneRepositoryButton.click();
    
    // Should eventually show timeout error
    await securityScannerPage.waitForToast('Failed to clone repository', 'error');
  });
  ...
  test('should handle special characters in repository URLs', async ({ page }) => {

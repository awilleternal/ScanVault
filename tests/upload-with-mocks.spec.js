...
test.describe('Upload Functionality with API Mocking', () => {
  ...
  test('should successfully upload a file with mocked API', async ({
    page,
  }) => {
  ...
  test('should handle file upload API error', async ({ page }) => {
    ...
    await expect(
      page
        ...
        .or(page.locator('text=Failed to upload file'))
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should show loading state during upload', async ({ page }) => {
    ...
    await expect(
      page
        .locator('text=Uploading file')
        .or(page.locator('text=loading').or(page.locator('[role="status"]')))
  ...
  test('should validate file type client-side', async ({ page }) => {
    ...
    try {
      ...
      await expect(
        page
          .locator('text=Please upload a ZIP file')
          .or(page.locator('text=Only ZIP files are allowed'))
      ).toBeVisible({ timeout: 5000 });
    } catch (error) {
  ...
  test('should handle repository URL with mocked API', async ({ page }) => {
    ...
    try {
      await expect(
        page.locator('text=Repository cloned successfully')
      ).toBeVisible({ timeout: 10000 });
    } catch (e) {
  ...
  test('should handle repository clone error', async ({ page }) => {
    ...
    await expect(
      page
        .locator('text=Repository not found')
        .or(page.locator('text=Failed to clone repository'))
    ).toBeVisible({ timeout: 10000 });
  });

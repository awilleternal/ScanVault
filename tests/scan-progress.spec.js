...
test.describe('Scan Progress Monitoring', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    if (testInfo.status !== testInfo.expectedStatus) {
      await takeTimestampedScreenshot(page, `scan-progress-failure-${testInfo.title}`);
    }
  ...
  test('should display progress monitoring interface', async ({ page }) => {
    ...
    await expect(
      securityScannerPage.currentToolIndicator.or(securityScannerPage.progressBar)
    ).toBeVisible();
  ...
  test('should show progress updates via WebSocket', async ({ page }) => {
    ...
    const progressUpdates = [
      { currentTool: 'Semgrep', progressPercent: 25, message: 'Starting Semgrep analysis...' },
      { currentTool: 'Semgrep', progressPercent: 50, message: 'Semgrep scanning files...' },
      { currentTool: 'Trivy', progressPercent: 75, message: 'Starting Trivy scan...' },
      { currentTool: 'Trivy', progressPercent: 100, message: 'Trivy scan completed' }
    ];
    ...
    await expect(page.locator('text=/Scanning|Progress|Running/')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should handle scan completion and show results', async ({ page }) => {
    ...
    await expect(securityScannerPage.resultsSection).toBeVisible({ timeout: 30000 });
  });

  test('should show fallback progress when WebSocket fails', async ({ page }) => {
    ...
    await expect(page.locator('text=/Scanning|Progress|%/')).toBeVisible({ timeout: 15000 });
    
    // Progress should advance over time
    await page.waitForTimeout(3000);
    
    // Should show some progress indication
    const progressElements = page.locator('.bg-primary-600, .bg-blue-600, text=/\d+%/');
  ...
  test('should show progress logs or messages', async ({ page }) => {
    ...
    const logElements = page.locator('pre, .font-mono, .bg-gray-100, text=/logs|output|scanning/i');
  ...
  test('should handle scan errors gracefully', async ({ page }) => {
    ...
    await expect(
      page.locator('text=/error|failed|problem/i').or(securityScannerPage.resultsSection)
  ...
  test('should show estimated time remaining', async ({ page }) => {
    ...
    const hasTimeEstimate = await timeElements.first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should maintain progress state during page interactions', async ({ page }) => {
  ...
  test('should handle browser refresh during scan', async ({ page }) => {
    ...
    const progressVisible = await securityScannerPage.progressSection.isVisible({ timeout: 5000 }).catch(() => false);
    const uploadVisible = await securityScannerPage.dropzone.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should show progress bar or percentage indicator', async ({ page }) => {
    ...
    const progressBars = page.locator('.progress, .bg-primary-600, .bg-blue-600, [role="progressbar"]');
    const percentageText = page.locator('text=/%|percent/i');
    
    // Should have some form of progress indication
    await expect(
      progressBars.first().or(percentageText.first()).or(page.locator('text=/progress/i'))
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should handle concurrent scan prevention', async ({ page }) => {
    ...
    await expect(
      securityScannerPage.dropzone.or(page.locator('text=/cancel|stop|confirm/i'))

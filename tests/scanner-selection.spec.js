...
import { createTestZipFile, cleanupTestFiles, takeTimestampedScreenshot, mockApiResponse } from './utils/testHelpers.js';
...
test.describe('Scanner Selection Workflow', () => {
  ...
  test.afterEach(async ({ page }, testInfo) => {
    ...
    if (testInfo.status !== testInfo.expectedStatus) {
      await takeTimestampedScreenshot(page, `scanner-selection-failure-${testInfo.title}`);
    }
  ...
  test('should display all available security scanners', async ({ page }) => {
    ...
    await expect(page.locator('h2', { hasText: 'Select Security Scanners' })).toBeVisible();
    await expect(page.locator('text=Choose which security tools to run')).toBeVisible();
  ...
  test('should display scanner information and metadata', async ({ page }) => {
    ...
    await expect(semgrepCard.locator('text=Static analysis tool')).toBeVisible();
    await expect(semgrepCard.locator('text=2-5 minutes')).toBeVisible();
    await expect(semgrepCard.locator('text=Requires WSL2 on Windows')).toBeVisible();
  ...
  test('should select and deselect individual scanners', async ({ page }) => {
    ...
    // Select Semgrep
    await page.locator('text=Semgrep').locator('..').locator('input[type="checkbox"]').check();
    await expect(page.locator('text=Selected tools: 1')).toBeVisible();
    await expect(securityScannerPage.startScanButton).toBeEnabled();
    
    // Select Trivy
    await page.locator('text=Trivy').locator('..').locator('input[type="checkbox"]').check();
    await expect(page.locator('text=Selected tools: 2')).toBeVisible();
    
    // Deselect Semgrep
    await page.locator('text=Semgrep').locator('..').locator('input[type="checkbox"]').uncheck();
    await expect(page.locator('text=Selected tools: 1')).toBeVisible();
    
    // Deselect all
    await page.locator('text=Trivy').locator('..').locator('input[type="checkbox"]').uncheck();
    await expect(page.locator('text=Selected tools: 0')).toBeVisible();
    await expect(securityScannerPage.startScanButton).toBeDisabled();
  });
  ...
  test('should support select all functionality', async ({ page }) => {
    ...
    for (const tool of expectedScannerTools) {
      const checkbox = page.locator('text=' + tool).locator('..').locator('input[type="checkbox"]');
      await expect(checkbox).toBeChecked();
    }
    
    // Selected count should match total tools
    await expect(page.locator(`text=Selected tools: ${expectedScannerTools.length}`)).toBeVisible();
    await expect(securityScannerPage.startScanButton).toBeEnabled();
    
    // Selected tools should be displayed as tags
    for (const tool of expectedScannerTools) {
      await expect(page.locator('.bg-primary-100').filter({ hasText: tool })).toBeVisible();
    }
  });
  ...
  test('should calculate and display estimated scan time', async ({ page }) => {
    // No tools selected
    await expect(page.locator('text=Estimated total time: 0 minutes')).toBeVisible();
    
    // Select one tool
    await page.locator('text=Semgrep').locator('..').locator('input[type="checkbox"]').check();
    await expect(page.locator('text=Estimated total time: 2-5 minutes')).toBeVisible();
    
    // Select all tools
    await securityScannerPage.selectAllButton.click();
    const expectedMin = expectedScannerTools.length * 2;
    const expectedMax = expectedScannerTools.length * 5;
    await expect(page.locator(`text=Estimated total time: ${expectedMin}-${expectedMax} minutes`)).toBeVisible();
  });

  test('should handle card click selection', async ({ page }) => {
    // Click on scanner card (not checkbox) should toggle selection
    const semgrepCard = page.locator('text=Semgrep').locator('..').locator('..');
    
    // Click card to select
    await semgrepCard.click();
    const checkbox = page.locator('text=Semgrep').locator('..').locator('input[type="checkbox"]');
    await expect(checkbox).toBeChecked();
  ...
  test('should show visual feedback for selected scanners', async ({ page }) => {
    const semgrepCard = page.locator('text=Semgrep').locator('..').locator('..');
    
    // Initially not selected
    await expect(semgrepCard).not.toHaveClass(/border-primary-500|bg-primary-50/);
  ...
  test('should show validation message when trying to start without selection', async ({ page }) => {
    ...
    await securityScannerPage.waitForToast('Please select at least one security tool', 'error');
  });

  test('should successfully start scan with selected tools', async ({ page }) => {
  ...
  test('should maintain selection state during interaction', async ({ page }) => {
    // Select some tools
    await page.locator('text=Semgrep').locator('..').locator('input[type="checkbox"]').check();
    await page.locator('text=Trivy').locator('..').locator('input[type="checkbox"]').check();
    ...
    await expect(page.locator('text=Selected tools: 2')).toBeVisible();
    const semgrepCheckbox = page.locator('text=Semgrep').locator('..').locator('input[type="checkbox"]');
    const trivyCheckbox = page.locator('text=Trivy').locator('..').locator('input[type="checkbox"]');
    await expect(semgrepCheckbox).toBeChecked();
    await expect(trivyCheckbox).toBeChecked();
  });

  test('should handle scanner selection with keyboard navigation', async ({ page }) => {
  ...
  test('should display selected tools as chips/tags', async ({ page }) => {
    ...
    await expect(page.locator('.bg-primary-100').filter({ hasText: 'Semgrep' })).toBeVisible();
    await expect(page.locator('.bg-primary-100').filter({ hasText: 'Trivy' })).toBeVisible();
    
    // Chips should have icons
    await expect(page.locator('.bg-primary-100').filter({ hasText: 'Semgrep' }).locator('text=🔍')).toBeVisible();
    await expect(page.locator('.bg-primary-100').filter({ hasText: 'Trivy' }).locator('text=🛡️')).toBeVisible();
  });

...
function showPathInstructions() {
  ...
  console.log(
    'ODC_PATH=C:\\tools\\dependency-check\\bin\\dependency-check.bat'
  );
...
setTimeout(async () => {
  ...
  try {
    ...
    console.log(
      `ODC Bridge Detection: ${available ? '✅ Success' : '❌ Failed'}`
    );

    if (!available) {
      console.log('\n💭 Troubleshooting:');
      console.log(
        '- Check that dependency-check.bat --version works in your terminal'
      );
